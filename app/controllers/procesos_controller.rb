class ProcesosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @proceso.save
        format.html { redirect_to procesos_url, notice: 'Proceso was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @proceso.update(proceso_params)
        format.html { redirect_to procesos_url, notice: 'Proceso was successfully updated.' }
  ...
  private
    ...
    def proceso_params
      params.require(:proceso).permit(:name, :description, :admin_user, :user_id, :proces_type, :prefix)
    end
end
