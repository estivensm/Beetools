...
class FieldsTest < ApplicationSystemTestCase
  ...
  test "creating a Field" do
    ...
    fill_in "Admin user", with: @field.admin_user
    fill_in "Field type", with: @field.field_type
    fill_in "Form", with: @field.form
    fill_in "Name", with: @field.name
    check "Required" if @field.required
    check "State" if @field.state
    fill_in "User", with: @field.user_id
